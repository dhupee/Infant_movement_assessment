...
def main(data_set):
    ...
    df = pd.read_pickle(os.path.join(
        pose_estimates_path, 'pose_estimates.pkl'))
    ...
    df = df.groupby(['video', 'bp']).apply(
        interpolate_df).reset_index(drop=True)
    # median and mean filter
    median_window = .5
    mean_window = .5

    # dropping index with NaN value
    df = df.dropna().groupby(['video', 'bp']).apply(
        lambda x: smooth(x, 'y', median_window, mean_window)).reset_index(drop=True)
    df = df.dropna().groupby(['video', 'bp']).apply(
        lambda x: smooth(x, 'x', median_window, mean_window)).reset_index(drop=True)
    ...
    # save
    xdf.to_pickle(os.path.join(pose_estimates_path,
                  'processed_pose_estimates_coords.pkl'))
    adf.to_pickle(os.path.join(pose_estimates_path,
